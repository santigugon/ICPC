...
struct Segtree{
  ...
  struct Node{
    lli s, mx, mn, smn;//-------
    Node(lli s = 0, lli mx = -INF, lli mn = INF, lli smn=0): s(s), mx(mx), mn(mn), smn(smn) {}//---------
    Node operator + (const Node &n){
        
        
        lli nsmn = smn; 
        if(mn==n.mn)nsmn+=n.smn;
        if(mn>n.mn)nsmn=n.smn;
        
        // else if(n.mn==min(mn,n.mn))smn++;
      
        
      return Node(s + n.s, max(mx, n.mx), min(mn,n.mn),nsmn);
